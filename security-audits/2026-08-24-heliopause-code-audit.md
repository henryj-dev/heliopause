...
- 현재 후속 검토 커밋: `f6ef42f` — M-01/L-02/I-02 수정과 감사 문서·기대값 갱신을 포함
...
그 밖에 **저장소 밖에서 등록 확인이 끝난 항목 1건**(O-01)이 있다. 실제 강제 로그아웃
전달·세션 종료 여부는 운영 저널로 별도 확인해야 한다.
...
### O-01. IdP Back-channel Logout URI 등록 확인 및 운영 검증 대기

- 상태: **코드 완비 · URI 등록 확인됨 · 실제 전달 검증 대기**
...
KeyStone 운영 확인으로 Back-channel Logout URI 등록이 완료됐다. 배포 체크리스트
(`docs/oidc-배포-체크리스트.md`)에는 `post_logout_redirect_uri`도 등록 목록에 있다.
따라서 O-01은 미등록 항목이 아니라, 실제 강제 로그아웃 전달과 세션 종료 저널을 확인할
운영 검증 항목으로 상태를 변경한다.
...
여기서 실제로 남은 운영 확인은 강제 로그아웃 요청을 한 번 발생시키는 것이며,
...
등록은 확인됐으므로, 이제 저널에 성공 줄이 남는지 확인한다. 거부 줄이 나오면 URI 등록은
됐고 토큰이 거부된 것이며, 그 줄이 이유를 말한다.
