...
CASES = [
    ...
    # ⚠️ 추적되는 경로여야 한다. 이 케이스는 `{MAIN}/docs/x.md` 를 쓰다가 0567c5e 이후로
    #    **거짓 실패**가 됐다 — 그 커밋이 「무시되고 추적도 안 되는 경로는 막지 않는다」를
    #    넣었고 `docs/` 가 정확히 그것이라, 가드가 설계대로 통과시키는데 검사만 옛 답을
    #    들고 있었다. 그 예외의 양성은 `test-ignored-paths.py` 가 따로 든다.
    ("메인에서 Write",                     "S1", MAIN,   "Write", {"file_path": f"{MAIN}/README.md"},      "DENY"),
    ...
    # 같은 이유로 추적되는 경로를 쓴다 — 여기서 물어야 하는 것은 「모르는 이름 + 내용」이
    # 잡히는가이지, 무시 예외가 아니다.
    ("모르는 이름 + path/content 도 막는다",  "S1", MAIN,   "write_file", {"path": f"{MAIN}/README.md", "content": "x"}, "DENY"),
    # 🔑 그 예외가 여기서도 보이게 남긴다. 위 둘을 `docs/` 로 되돌리는 「수정」이 다시
    #    나오지 않도록, 통과해야 한다는 사실을 같은 목록에 적는다.
    ("무시되고 추적도 안 되는 경로는 통과",     "S1", MAIN,   "Write", {"file_path": f"{MAIN}/docs/x.md"},      "ALLOW"),
