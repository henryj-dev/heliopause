...
`npm test` 1,855 + 8 (`@heliopause/manager`) + 206 (`@heliopause/web`) ·
`test_validate.py` 255 (실행 243 · skip 12) · `test_enroll.py` 16.
그 1,855 는 **이 저장소 1,769(`src` + `examples`) + `policy` 86** 이다. 둘로 나눠 적는 이유는
