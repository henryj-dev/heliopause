...
`npm test` 2,002 + 8 (`@heliopause/manager`) + 206 (`@heliopause/web`) ·
`test_validate.py` 272 (리눅스에서 실행 272 · macOS 에서 skip 12) · `test_enroll.py` 16.
그 2,002 는 **이 저장소 1,915(`src` + `examples`) + `policy` 87** 이다. 둘로 나눠 적는 이유는
바로 아래에 있다 — 뒤의 87 은 이 저장소의 코드를 안 읽는다.

한동안 「1,800 + 93」이라고 적혀 있었다. **둘 다 틀렸고, 합이 맞아서 아무도 안 봤다** — 바로
아래 「87개가 말없이 사라진다」와도 어긋나 있었는데 그것도 그대로 지나갔다. 그러니 이 수를
고칠 때는 **합만 맞추지 말고 양쪽을 각각 재라**(2026-09-04 실측).
