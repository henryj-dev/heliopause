...
| `policy` | `npm test` 가 `policy/*.test.ts` 를 돌린다 — 없으면 **테스트가 깨진다.** 자체 git 저장소다 |
| `docs` | 배포 설계 기록을 못 읽는다 (코드는 참조하지 않는다) |
| `pki-prod` · `pki-signing` | `heliopause-publish` · `heliopause-approve` 흐름을 워크트리에서 못 돌린다 |
| `pki` · `pki-util` | **함대를 못 읽는다.** `pki/` 는 **dev** VPC 이고, 매니저는 클라이언트 인증서를 그 CA(`HELIOPAUSE_CA_FILE=/pki/dev/ca.pem`)로 검증한다 — 즉 `pki/operator-ops-henry.pem` 이 `GET /api/site` 로 **세 VPC 를 한꺼번에** 읽는 유일한 열쇠다 |

⚠️ **CA 는 VPC 마다 하나고 이름이 전부 `CN=heliopause-ca` 다.** 그래서 엉뚱한 것을 들이대도
「이름이 맞으니 되겠지」로 읽힌다 — 2026-08-18 에 `pki-prod/operator-ops-henry.pem` 으로 매니저를
불렀더니 `no client certificate and no session` 이 돌아왔다. 거절이 아니라 **핸드셰이크에서 아예
안 실린 것**이고, 증상은 「인증서를 안 보냈다」라 클라이언트가 고장 난 것처럼 보인다.
가르는 방법은 이름이 아니라 지문이다:

```bash
openssl x509 -in pki/ca.pem -noout -fingerprint -sha256
kubectl -n heliopause exec deploy/heliopause-manager -- openssl x509 -in /pki/dev/ca.pem -noout -fingerprint -sha256
```
